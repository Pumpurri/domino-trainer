...
The 48-position public-information stratification study produced a promising middle-game signal but failed its locked sample-use check. A 60-position fixed-budget follow-up rejected it at the 120-sample live-coach budget but passed every 500-sample check. The final 160-position Deep Review-only holdout then passed 11 of 12 gates: regret, two-reference quality, labels, false accusations, runtime, effective samples, exact budgets, and phase safety all improved or remained controlled. Repeat acceptability stayed exactly 47% instead of improving, so the candidate failed its preregistered gate and no product behavior changed. The stratification line is now closed unless a materially different method targets recommendation stability.
