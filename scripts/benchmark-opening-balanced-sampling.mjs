...
const LABELS = {
  ...
  'opening-response-balanced-35': 'Response balanced 35%',
  'opening-response-balanced-60': 'Response balanced 60%',
  'opening-return-balanced-35': 'Response plus return balanced 35%',
  'opening-return-balanced-60': 'Response plus return balanced 60%',
};
...
function variantTable(summary) {
  const rows = [
    ...
    ...OPENING_BALANCED_CANDIDATES.map((candidate) => [candidate, summary.candidates[candidate].candidate]),
  ];
  return [
...
function renderReport(config, summary) {
  return `# Mesa Quince opening-balanced sampling development study
...
${OPENING_BALANCED_CANDIDATES.map((candidate) => candidateSection(candidate, summary.candidates[candidate])).join('\n\n')}
`;
}
...
const seed = argument('seed') ?? 'mesa-quince-opening-balanced-development-v1';
...
console.log('MESA QUINCE OPENING-BALANCED SAMPLING STUDY');
...
const corpus = collectOpeningBalancedCorpus({ positions, minimumBranching, seed });
...
const fresh = pending.length ? await evaluatePositionsParallel({
  ...
  workerCount: workers,
  workerUrl: new URL('./opening-balanced-sampling-worker.mjs', import.meta.url),
...
const summary = summarizeOpeningBalanced(results, { seed, confidenceResamples, budget });
